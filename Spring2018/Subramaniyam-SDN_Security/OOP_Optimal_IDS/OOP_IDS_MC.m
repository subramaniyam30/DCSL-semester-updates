...
% node 1 can't be crown jewel
% edge servers can't be crown jewel as benefit is 0 at those locations. the
% benefit is defined as P(A_in at crown jewel)
classdef OOP_IDS_MC < handle
    ...
    properties
        ...
        crownJewel                % The index of all the crownjewel nodes.
        
        numCrownJewels            % number of crown jewel nodes.
        
        IDSquality                % IDS Security Value for each Server Node. Array with each index representing the server node number.
        
        weight = 0.5;             % importance of relative cost  wrt performance -> (0 < weight < 1)
        %cost_budget               % max cost allowed/budget - cost contrainst. numerical value.
        budget_IDS                 % Maximum No. of IDS allowed.
    ...
    properties(Constant)
        ...
        %maxTrafAllowed = 5000e3; % Max traffic that can be handled by a single IDS
        
    end
    
    methods
        %constructor called first.
        function obj=OOP_IDS_MC(numCrownJewels,graphKey,servers,connections)
            if nargin < 2 || isempty(graphKey)            %if the no. of input parameters are less than 1. Default consider 3rd Toy Network.
                graphKey=3;
            end
...
%             % First Node and Last Node not included in the calculations as
%             % the IDS value is 0. can't be placed there.
%             maxTrafCost = 0;
...
%         end
        ...
        % max benefit for all the crown jewels will be minimum of max
        % benefit out of each crown jewel.
        function calculate_max_benefit(obj)
            for i=1:obj.numCrownJewels
             benefit (i) = (obj.attackerTrafInNoIDS(obj.crownJewel(i)) - obj.trafInMaxIDS(obj.crownJewel(i))) ...  
                /obj.attackerTrafInNoIDS(obj.crownJewel(i));
            end
            obj.maxBenefit = min(benefit);
        ...
        function [finalIDSPlacement,runningTime] = optimizeWithGA(obj)
            ...
            ub= ones((obj.numServers),1);  % each element in placement vector is upper bounded by 1 except first node.
        ...
        % benefit is defined as minimum benefit out of all  the CJ.
        function benefit = calculate_benefit(obj,placement)
            ...
            for i=1:obj.numCrownJewels
            benefit_cj(i) = (obj.attackerTrafInNoIDS(obj.crownJewel(i)) - trafInIDS(obj.crownJewel(i)))/obj.attackerTrafInNoIDS(obj.crownJewel(i));
            %disp(benefit_cj(i));
            end
            
            benefit = min(benefit_cj);
            obj.average_benefit = benefit;
            %obj.average_benefit = sum(benefit_cj)/obj.numCrownJewels;
            %benefit = sum(benefit_cj);
        end
        
        function benefit = calculate_benefit_exhaustive(obj,placement)
            % Benefit = reduction in probability of attacker succeeding
            % Send the IDS quality values for only those nodes where there
            % is an IDS.
            protection = obj.IDSquality.*placement;
        ...
        % Cost Constraint function for GA optimization. 
        % budget is taken as 108.
        % All the placements with total cost > budget is not considered.
        % GA chooses only placements whose c < =  0; => only those for
        % which cost placement < budget.
%         function [c,ceq] = cost_constraint(obj,placement)
%             c = obj.calculate_num_cost(placement) - obj.cost_budget;   % finds the cost for that particular configuration.
%             %disp(c);
%             ceq = [];
%         end
        
        
%         % Calculate the total cost for that particular placement vector.
%         function numCost = calculate_num_cost(obj,placement)
%             % Cost = installation cost + operation/running cost + traffic latency cost
%             
%             % Cost of placing IDS at node
%             installCost = obj.unitCost*sum(placement == 1); % installation costs
%             operateCost = obj.runCost*sum(placement == 1); % operating/running costs
%             
...
%                         trafCost = 1000;  % when the input traffic is more than what an IDS can handle.
%                     else
...
%             if obj.calculate_num_cost(placement) >= obj.cost_budget
%                 placeCost = 1000;
%             else
%                 placeCost = 0;
%             end
...
%         end   
        ...
        % (1-weight)*performance - (weight*relative cost);
...
%         
        ...
        function multiObjFunc = moo_objective_function(obj,placement)
         ...
         function [finalIDSPlacement,runningTime] = optimizeWithExhSearchMOO(obj)
            ...
            testSet=[];
            for i=1:obj.numServers
                setToOnes=combnk(1:(obj.numServers-1),i);
                for j=1:size(setToOnes,1)
                    setVector=setToOnes(j,:);
                    newTestSet=zeros(obj.numServers,1);
                    newTestSet(setVector)=1;
       ...
       function path=getRandomPath(obj)
        ...
        function updateEdgeWeight(obj,alertNode)
    ...
    methods(Static)
        ...
        function [network,crownJewel] = toyNetwork(numCrownJewels,key)
            if key == 1
            ...
            elseif key == 3
                ...
                network = digraph(s,t);
                %crownJewel = 12;
                % randomly select crown jewels from 12 nodes.
                cj_vector = [5,6,7,8,9,10,11,12];
                crownJewel = randsample(cj_vector,numCrownJewels);
            elseif key == 4
